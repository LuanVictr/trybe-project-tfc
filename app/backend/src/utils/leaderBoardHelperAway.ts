...
class LeaderboardHelperAway {
  constructor(private matchesModel = Matches, private teamsModel = Teams) {}

  async getLeaderBoardAway(id:number) {
    const teamInfo = await this.teamsModel.findOne({ where: { id } });
    const matchInfo = await this.matchesModel.findAll({ where: { awayTeamId: id, inProgress: 0 } });
    return {
  ...
  async calculateGoalsFavor(id:number):Promise<number> {
    let goalsFavor = 0;
    const matchInfo = await this.matchesModel.findAll({ where: { awayTeamId: id, inProgress: 0 } });
    matchInfo.forEach((match) => { goalsFavor += match.awayTeamGoals; });
    return goalsFavor;
  }
  ...
  async calculateGoalsOwn(id:number):Promise<number> {
    let goalsOwn = 0;
    const matchInfo = await this.matchesModel.findAll({ where: { awayTeamId: id, inProgress: 0 } });
    matchInfo.forEach((match) => { goalsOwn += match.homeTeamGoals; });
    return goalsOwn;
  }

  async calculateVictories(id:number):Promise<number> {
    let totalVictories = 0;
    const result = await this.matchesModel.findAll({ where: { awayTeamId: id, inProgress: 0 } });
...
export default LeaderboardHelperAway;
